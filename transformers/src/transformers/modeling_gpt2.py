...
class GPT2LMHeadModel(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        a=-1
        b=0
        for u in range(input_ids.size(1)):
            if input_ids[0,u]==DELIM_ID:
                a=u
                b=1
            elif input_ids[0,u]==DELIM_ID2:
                delimc.append([a,u])
                b=0
        if b==1:
            delimc.append([a,input_ids.size(1)])
        ...
        if labels is not None:
            ...
            if len(delimc)>0:
                aa1=shift_logits.view(-1, shift_logits.size(-1))[delimc[0][0]:delimc[0][1],:]
                aa2=shift_labels.view(-1)[delimc[0][0]:delimc[0][1]]
                for indices in delimc[1:]:
                    aa1=torch.cat((aa1,shift_logits.view(-1, shift_logits.size(-1))[indices[0]:indices[1],:]),0)
                    aa2=torch.cat((aa2,shift_labels.view(-1)[indices[0]:indices[1]]),0)
                for inde in range(10):
                    aa1=torch.cat((aa1,shift_logits.view(-1, shift_logits.size(-1))[delimc[0][0]+1:delimc[0][0]+2,:]),0)
                    aa2=torch.cat((aa2,shift_labels.view(-1)[delimc[0][0]+1:delimc[0][0]+2]),0)
                if aa1.size(0)==0:
                    loss = 0
                else:
                    loss = loss_fct(aa1,aa2)
            else:
                loss=0 
